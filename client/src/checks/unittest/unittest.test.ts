...
const pathToImport = join(
  __dirname,
  "../../../data/unittest"
)
const dirImport = directory(pathToImport);

it("imports", () => {
  const ut = unittest(dirImport);
  const utJSON = (JSON.stringify(ut, null, 2));
  writeFileSync(
    join(__dirname, "../../../data/unittest.json"),
    utJSON);
});
